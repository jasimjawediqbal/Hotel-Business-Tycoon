...
public class Customer {
    ...
    public void adjustSatisfaction(int change) {
        ...
        if (satisfactionLevel > 100) satisfactionLevel = 100;
        if (satisfactionLevel < 0) satisfactionLevel = 0;
    }
    
    public int getReputationImpact() {
        if (!willLeaveReview) return 0;
        
        if (satisfactionLevel >= 80) return 3;
        else if (satisfactionLevel >= 60) return 1;
        else if (satisfactionLevel >= 40) return 0;
        else if (satisfactionLevel >= 20) return -1;
        else return -3;
    }
    ...
    public boolean willLeaveReview() { return willLeaveReview; }
}
...
public class ReceptionPanel extends JPanel {
    ...
    // Animation
    private ArrayList<AnimatedCustomer> walkingCustomers = new ArrayList<>();
    private ArrayList<CustomerRequest> pendingRequests = new ArrayList<>();
    ...
    private RequestsTabPanel requestsTab;
    private GuestsTabPanel   guestsTab;
    private RoomTabPanel     roomTab;
    private EmployeeTabPanel employeeTab;
    private HistoryTabPanel  historyTab;
    ...
    public ReceptionPanel(JFrame parent) {
        ...
        startAnimation();
        startDayTimer();
        SwingUtilities.invokeLater(() -> showChapterIntro(1));
    }
    ...
    private void initializeUI() {
        ...
        requestsTab  = new RequestsTabPanel();
        guestsTab    = new GuestsTabPanel();
        roomTab      = new RoomTabPanel();
        employeeTab  = new EmployeeTabPanel();
        historyTab   = new HistoryTabPanel();
    ...
    private JPanel createStatsPanel() {
        ...
        dayLabel.setFont(f);        dayLabel.setForeground(Color.WHITE);
        timeLabel.setFont(f);       timeLabel.setForeground(new Color(150, 200, 255));
        moneyLabel.setFont(f);      moneyLabel.setForeground(new Color(100, 255, 100));
        reputationLabel.setFont(f); reputationLabel.setForeground(new Color(255, 215, 100));

        panel.add(dayLabel); panel.add(timeLabel); panel.add(moneyLabel); panel.add(reputationLabel);
        return panel;
    }
    ...
    private JPanel createModernTabBar() {
        ...
        bar.add(requestsTabBtn); bar.add(guestsTabBtn); bar.add(roomsTabBtn);
        bar.add(employeesTabBtn); bar.add(historyTabBtn);
        return bar;
    }
    ...
    private JButton createTabButton(String text, String tabName) {
        ...
        btn.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                if (!tabName.equals(currentOpenTab)) btn.setBackground(new Color(120, 120, 130));
            }
            public void mouseExited(MouseEvent e) {
                if (!tabName.equals(currentOpenTab)) btn.setBackground(inactiveTabColor);
            }
        });
        return btn;
    }
    ...
    private JButton getTabBtn(String name) {
        switch(name) {
            case "requests":  return requestsTabBtn;
            case "guests":    return guestsTabBtn;
            case "rooms":     return roomsTabBtn;
            case "employees": return employeesTabBtn;
            default:          return historyTabBtn;
        }
    }

    private void refreshTab(String name) {
        switch(name) {
            case "requests":  requestsTab.refresh();  break;
            case "guests":    guestsTab.refresh();    break;
            case "rooms":     roomTab.refresh();      break;
            case "employees": employeeTab.refresh();  break;
        }
    }
    ...
    private void styleBtn(JButton b) {
        ...
        b.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) { b.setBackground(new Color(100,150,200)); }
            public void mouseExited(MouseEvent e)  { b.setBackground(new Color(70,130,180)); }
        });
    }
...
private void generateCustomerRequest() {
    ...
    //     else if (cond == RoomCondition.POOR) request.customer.adjustSatisfaction(-10);
    //     else if (cond == RoomCondition.VERY_POOR) request.customer.adjustSatisfaction(-25);

    //     historyTab.addLog("✅ " + request.customer.getName() + " → Room " + request.room.getRoomNumber()
    ...
    for (AnimatedCustomer ac : walkingCustomers) {
        if (ac.linkedRequest == request) { ac.accept(); break; }
    }
...
public void rejectRequest(CustomerRequest request) {
    ...
    for (AnimatedCustomer ac : walkingCustomers) {
        if (ac.linkedRequest == request) { ac.reject(); break; }
    }
...
private void addWalkingCustomer(Customer guest, int roomNumber, CustomerRequest request) {
    walkingCustomers.add(new AnimatedCustomer(guest, roomNumber, request));
}

    // ==================== STORY ====================

    private void showStoryEvent() {
        if      (currentChapter == 1 && !chapter1Complete) showChapter1Events();
        else if (currentChapter == 2 && !chapter2Complete) showChapter2Events();
        else if (currentChapter == 3 && !chapter3Complete) showChapter3Events();
        else JOptionPane.showMessageDialog(this, "No story events available.\nKeep managing your hotel!",
    ...
    private void showChapter3Events() {
        JOptionPane.showMessageDialog(this,
            ...
            "Chapter 3", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void checkChapter1Progress() {
        if (!chapter1Complete && hotel.getUsableRoomCount() >= 3 &&
            ...
            chapter1Complete = true; currentChapter = 2;
            SwingUtilities.invokeLater(() -> { showChapterComplete(1); onChapterComplete(1); });
        }
    }

    private void checkChapterProgress() {
        checkChapter1Progress();
        if (chapter1Complete && !chapter2Complete && hotel.getReputation() >= 40 && hotel.getMoney() >= 800) {
            chapter2Complete = true; currentChapter = 3;
            SwingUtilities.invokeLater(() -> { showChapterComplete(2); onChapterComplete(2); });
        }
        if (chapter2Complete && !chapter3Complete && hotel.getEmployees().size() >= 2 && hotel.getReputation() >= 60) {
            chapter3Complete = true;
            SwingUtilities.invokeLater(() -> { showChapterComplete(3); onChapterComplete(3); });
        }
    }
    ...
    private void showChapterIntro(int ch) {
        String msg;
        switch(ch) {
            case 1: msg = "═══ CHAPTER 1: Homecoming ═══\n\n3 rooms functional, 1 broken\n$500 in hand\nDaily expenses: $50\n\n" +
                          "Requirements:\n• 3+ usable rooms\n• $300 total revenue\n• Meet Rashid\n• Find old register"; break;
            case 2: msg = "═══ CHAPTER 2: The Mystery ═══\n\nRooms doubled!\n\nRequirements:\n• 40+ reputation\n• $800+ money"; break;
            default: msg = "═══ CHAPTER 3: Growth ═══\n\nRooms doubled again!\n\nRequirements:\n• 2+ employees\n• 60+ reputation";
        }
    ...
    private void showChapterComplete(int ch) {
        ...
        if (ch + 1 <= 3) showChapterIntro(ch + 1);
    }
    ...
    private void refreshAllTabs() {
        ...
        if (currentOpenTab != null) refreshTab(currentOpenTab);
        backgroundPanel.updateRoomLights();
        updateTabBadge();
    }
    ...
    private void startAnimation() {
        animationTimer = new Timer(33, e -> { // ~30fps
            ...
            if (frameCount % 60 == 0) generateCustomerRequest();
            ...
            for (int i = pendingRequests.size() - 1; i >= 0; i--) {
                ...
                if (req.framesLived >= REQUEST_DURATION_FRAMES) {
                    ...
                    if ("requests".equals(currentOpenTab)) requestsTab.refresh();
                    updateTabBadge();
                    refreshAllTabs();
                }
            }
            ...
            for (int i = walkingCustomers.size() - 1; i >= 0; i--) {
                ...
                ac.update();
                if (ac.isFinished()) walkingCustomers.remove(i);
            }
    ...
    private void gameOver() {
        animationTimer.stop(); dayTimer.stop();
        JOptionPane.showMessageDialog(this,
    ...
    private void returnToMenu() {
        ...
        } else { animationTimer.start(); dayTimer.start(); }
    }

    // ==================== CUSTOMER REQUEST ====================

    class CustomerRequest {
        Customer customer; Room room; int nights, price;
        int framesLived = 0;

        CustomerRequest(Customer c, Room r, int n, int p) {
            customer = c; room = r; nights = n; price = p;
        }
    ...
    class ReceptionBackgroundPanel extends JPanel {
        ...
        public ReceptionBackgroundPanel() { setOpaque(true); }
        ...
        protected void paintComponent(Graphics g) {
            ...
            Color skyTop, skyBot;
            if (gameHour >= 6 && gameHour < 18)       { skyTop = new Color(135,206,235); skyBot = new Color(245,240,230); }
            else if (gameHour >= 18 && gameHour < 21)  { skyTop = new Color(255,140,60);  skyBot = new Color(200,100,80);  }
            else                                        { skyTop = new Color(20,20,60);    skyBot = new Color(40,40,80);    }
            ...
            if (gameHour < 6 || gameHour >= 20) {
                ...
                for (int i = 0; i < 40; i++) g2.fillOval(sr.nextInt(w), sr.nextInt(h/2), 2, 2);
            }
            ...
            for (int x = 0; x < w; x += 40)
                for (int y = h-150; y < h; y += 40)
                    if ((x/40 + y/40) % 2 == 0) g2.fillRect(x, y, 40, 40);

            // Hotel building - grows with more rooms
            int total   = hotel.getRooms().size();
            int bh      = Math.min(520, 200 + total * 25);
            drawBuilding(g2, w - 350, h - bh - 150, 300, bh);
            drawDesk(g2, 80, h - 230);

            for (AnimatedCustomer ac : walkingCustomers) ac.draw(g2);
        }
        ...
        private void drawDesk(Graphics2D g2, int x, int y) {
            g2.setColor(new Color(0,0,0,40)); g2.fillRoundRect(x+3,y+3,200,80,8,8);
            g2.setColor(new Color(139,90,60)); g2.fillRoundRect(x,y,200,80,8,8);
            g2.setColor(new Color(160,110,80)); g2.fillRoundRect(x-5,y,210,15,6,6);
            g2.setColor(new Color(50,50,50)); g2.fillRect(x+80,y-25,40,22);
            g2.setColor(new Color(100,150,200)); g2.fillRect(x+83,y-23,34,18);
            g2.setColor(new Color(70,50,40)); g2.fillRect(x+10,y+15,70,40);
            g2.setColor(new Color(220,200,150)); g2.setFont(new Font("Serif",Font.BOLD,12));
    ...
    private Color shirtColor;
    private float speed;
    private CustomerRequest linkedRequest; // which request this person belongs to
    ...
    public AnimatedCustomer(Customer customer, int roomNumber, CustomerRequest request) {
        this.customer       = customer;
        this.roomNumber     = roomNumber;
        this.linkedRequest  = request;
        this.x              = -50 - random.nextInt(80);
        this.y              = getHeight() - 195 + random.nextInt(20);
        this.speed          = 2.5f + random.nextFloat() * 1.5f;
        this.shirtColor     = new Color(
            80 + random.nextInt(176),
            80 + random.nextInt(176),
            80 + random.nextInt(176)
    ...
    public void reject() { state = 3; }

    /** True if request still pending and person is waiting */
    public boolean isWaiting() { return state == 1; }

    public void update() {
        switch (state) {
            case 0: // Walk to desk
                x += speed;
                if (x >= 130) state = 1; // arrived at desk, now wait
                break;
            case 1: // Waiting — do nothing, stand still
                // If the linked request was removed externally (expired),
                // start walking back
                if (!pendingRequests.contains(linkedRequest)) state = 3;
                break;
            case 2: // Accepted — walk toward hotel door (right side ~540px)
                x += speed + 1;
                if (x >= 540) state = 4; // reached door, start fade
                break;
    ...
    class RequestsTabPanel extends JPanel {
        ...
        void addRequest(CustomerRequest r) { refresh(); }
        ...
        void refresh() {
            ...
            if (pendingRequests.isEmpty()) {
                ...
                for (CustomerRequest req : new ArrayList<>(pendingRequests)) {
        ...
        private JPanel buildCard(CustomerRequest req) {
            ...
            JLabel roomLbl  = new JLabel("🏠 Room " + req.room.getRoomNumber() + " (" + req.room.getCondition().toString().replace("_"," ") + ")");
            ...
            info.add(nameLbl); info.add(roomLbl); info.add(nightLbl); info.add(priceLbl); info.add(timeLbl);
            ...
            acc.addActionListener(e -> { acceptRequest(req); refresh(); });
            rej.addActionListener(e -> { rejectRequest(req); refresh(); });
    ...
    class GuestsTabPanel extends JPanel {
        ...
        void refresh() {
            ...
            for (Room room : hotel.getRooms()) {
                ...
                txt.setOpaque(false); txt.add(nameLbl); txt.add(detLbl);
            ...
            if (!any) {
                ...
                e.setFont(new Font("SansSerif", Font.ITALIC, 14));
                e.setForeground(Color.GRAY); e.setAlignmentX(Component.CENTER_ALIGNMENT);
                listPanel.add(Box.createVerticalGlue()); listPanel.add(e);
            }
    ...
    class RoomTabPanel extends JPanel {
        ...
        void refresh() {
            ...
            for (Room room : hotel.getRooms()) {
                ...
                btn.setFocusPainted(false);
                btn.setCursor(new Cursor(Cursor.HAND_CURSOR));
                btn.setBackground(getColor(room.getCondition()));
                btn.setText("<html><div style='text-align:center;padding:3px'>" +
                    "<b style='font-size:13px'>Room " + rn + "</b><br>" +
                    "<span style='font-size:11px'>" + (room.isOccupied() ? "🔴 OCCUPIED" : "🟢 AVAILABLE") + "</span><br>" +
                    "<span style='font-size:10px;color:#555'>" + room.getCondition().toString().replace("_"," ") + "</span><br>" +
                    "<b style='color:#007700;font-size:12px'>$" + room.getBasePrice() + "/night</b>" +
                    "</div></html>");
                btn.addActionListener(e -> handleRoomClick(rn));
                roomGrid.add(btn);
            }
            roomGrid.revalidate(); roomGrid.repaint();
        }

        private Color getColor(RoomCondition c) {
            switch(c) {
                case BROKEN:    return new Color(240,130,130);
                case VERY_POOR: return new Color(250,180,130);
                case POOR:      return new Color(255,235,180);
                case GOOD:      return new Color(210,250,210);
                case EXCELLENT: return new Color(180,240,180);
                default:        return Color.LIGHT_GRAY;
            }
        }

        private void handleRoomClick(int rn) {
            Room room = hotel.getRooms().get(rn-1);
            switch(room.getCondition()) {
                case BROKEN:
                    if (confirm("Repair Room " + rn + " for $200?"))
                        if (hotel.repairRoom(rn, 200)) { historyTab.addLog("✓ Repaired Room "+rn+" ($200)"); refreshAllTabs(); }
                        else JOptionPane.showMessageDialog(this, "Not enough money!");
                    break;
                case VERY_POOR:
                    if (confirm("Repair Room " + rn + " for $100?"))
                        if (hotel.repairRoom(rn, 100)) { historyTab.addLog("✓ Repaired Room "+rn+" ($100)"); refreshAllTabs(); }
                        else JOptionPane.showMessageDialog(this, "Not enough money!");
                    break;
                case POOR:
                    if (confirm("Upgrade Room " + rn + " for $150?"))
                        if (hotel.repairRoom(rn, 150)) { historyTab.addLog("✓ Upgraded Room "+rn+" ($150)"); refreshAllTabs(); }
                        else JOptionPane.showMessageDialog(this, "Not enough money!");
                    break;
                default:
                    JOptionPane.showMessageDialog(this,
                        "Room " + rn + "\nCondition: " + room.getCondition() +
                        "\nStatus: " + (room.isOccupied() ? "Occupied – "+room.getNightsBooked()+" nights left" : "Available"),
...
add(scroll,      BorderLayout.CENTER);
        ...
        void refresh() {
            ...
            for (Employee emp : hotel.getEmployees()) {
                ...
                JLabel detLbl = new JLabel(em+" "+emp.getType().toUpperCase()+"  |  💵 $"+emp.getDailySalary()+"/day  |  ⚡ "+emp.getEfficiency()+"%");
                detLbl.setFont(new Font("SansSerif", Font.PLAIN, 11));
...
private void showHireDialog() {
    ...
    for (Employee e : hotel.getEmployees()) hiredNames.add(e.getName());
    ...
    hireBtn.addActionListener(e -> {
        ...
        hotel.hireEmployee(new Employee(name, type, salary));
        hotel.changeReputation(repBoost);
        historyTab.addLog("✅ Hired " + name + " as " +
    ...
    class HistoryTabPanel extends JPanel {
        ...
        HistoryTabPanel() {
            ...
            historyArea.setLineWrap(true); historyArea.setWrapStyleWord(true);
